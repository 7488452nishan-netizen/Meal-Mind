        ...
        <div className="max-w-5xl mx-auto animate-fadeIn">
            ...
            <div className="bg-white rounded-2xl shadow-card overflow-hidden border border-border">
                <div className="relative">
                    {isGeneratingImage ? (
                        <div className="w-full h-64 md:h-96 bg-muted flex flex-col items-center justify-center text-muted-foreground">
                        ...
                        <div className="relative" ref={shareRef}>
                            ...
                            {isShareOpen && (
                                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg z-10 animate-scaleIn origin-top-right border border-border">
                                    <button onClick={handleCopyLink} className="w-full text-left flex items-center space-x-2 px-4 py-2 hover:bg-muted"><Link2 className="w-4 h-4 text-muted-foreground"/><span>{t('details_copy_link')}</span></button>
                                    <button onClick={handleShareTwitter} className="w-full text-left flex items-center space-x-2 px-4 py-2 hover:bg-muted"><Twitter className="w-4 h-4 text-muted-foreground"/><span>{t('details_share_twitter')}</span></button>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="p-6 md:p-8">
                    <div className="flex flex-wrap gap-x-6 gap-y-4 justify-center text-center mb-8 pb-6 border-b border-border">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-1">
                             <h2 className="text-xl font-bold font-display mb-4 text-foreground">{t('details_ingredients')}</h2>
                                      ...
                                      return (
                                        <li key={i} className={`flex items-start ${hasIngredient ? '' : 'opacity-70'}`}>
                                            {hasIngredient ? 
                                                ...
                                                <div className="w-4 h-4 border-2 border-muted-foreground/50 rounded-full mr-2.5 mt-1 shrink-0"></div>
                                            }
                                            <span>{ing}</span>
                                        </li>
                                      )
                        ...
                        <div className="lg:col-span-2">
                             <h2 className="text-xl font-bold font-display mb-4 text-foreground">{t('details_instructions')}</h2>
                             <ol className="space-y-6 text-foreground/90 leading-relaxed">
...
const ImageRegenModal = ({ onClose, onGenerate, isPremium }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg animate-scaleIn relative">
                <button onClick={onClose} className="absolute top-4 right-4 p-2 text-muted-foreground hover:text-foreground"><X /></button>
                <h2 className="text-2xl font-bold text-center font-display mb-6">{t('details_regen_modal_title')}</h2>
                
                <div className="space-y-6">
                    <div>
                        <h3 className="font-semibold text-foreground mb-3">{t('details_regen_modal_style')}</h3>
                        <div className="flex flex-wrap gap-3">
                            {styles.map(s => (
                                <button key={s.id} onClick={() => setStyle(s.id)}
                                    className={`px-4 py-2 text-sm font-semibold rounded-full border-2 transition-colors duration-200 ${style === s.id ? 'bg-primary border-primary text-primary-foreground' : 'bg-white text-muted-foreground border-border hover:border-primary/50 hover:text-foreground'}`}>
                                    {s.name}
                                </button>
                            ))}
                        </div>
                    ...
                    {isPremium && (
                        <div>
                            <h3 className="font-semibold text-foreground mb-3">{t('details_regen_modal_aspect')}</h3>
                            <div className="flex flex-wrap gap-3">
                                {aspectRatios.map(ar => (
                                    <button key={ar} onClick={() => setAspectRatio(ar)}
                                        className={`px-4 py-2 text-sm font-semibold rounded-full border-2 transition-colors duration-200 ${aspectRatio === ar ? 'bg-primary border-primary text-primary-foreground' : 'bg-white text-muted-foreground border-border hover:border-primary/50 hover:text-foreground'}`}>
                                        {ar}
                                    </button>
                                ))}
                            </div>
                    ...
                    {!isPremium && <p className="text-sm text-muted-foreground text-center bg-muted p-3 rounded-lg">{t('details_regen_modal_premium_note')}</p>}
                </div>
